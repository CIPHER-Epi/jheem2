...
#' @param na.rm Setting this TRUE will pass 'na.rm=T' along to the pull function. If set to NULL, 'na.rm=T' will be used for outcomes with scale 'proportion', 'rate', or 'ratio'.
#'
...
JHEEM.BASIC.LIKELIHOOD.INSTRUCTIONS <- R6::R6Class(
    ...
    public = list(
        initialize = function(outcome.for.data,
            ...
            # *na.rm* must be NULL, T, or F
            if (!is.null(na.rm) && !(identical(na.rm, T)) && !(identical(na.rm, F)))
                stop(paste0(error.prefix, "'na.rm' must be NULL, TRUE, or FALSE"))
    ...
    active = list(
        ...
        na.rm = function(value) {
            if (missing(value)) {
                ...
                stop("Cannot modify a jheem.basic.likelihood.instruction's 'na.rm' - it is read-only")
            }
        }
    ),
    private = list(
...
JHEEM.BASIC.LIKELIHOOD <- R6::R6Class(
    ...
    public = list(
        initialize = function(instructions,
            ...
            # Decide na.rm if NULL
            na.rm = instructions$na.rm
            if (is.null(na.rm)) {
                if (scale %in% c('rate', 'ratio', 'proportion'))
                    na.rm=T
                else
                    na.rm=F
            }
